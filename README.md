...
| 확인 항목 | 없으면 |
|---|---|
| 이미지 생성 AI 접근(Gemini·네이토 등) | 무료 계정으로 시작 가능 |
| 영상 생성 도구(Sora 2 / Veo / Pika / Kling 중 1개) | 학습 플랫폼 내장 도구 사용 |
| 음성 합성 도구(TTS) | 네이토 TTS 또는 무료 Edge-TTS |
| ffmpeg (컷 연결·오디오 합치기) | `winget install ffmpeg` / `brew install ffmpeg` |
| 기본 PC 조작 (파일 저장·경로 확인) | 운영체제 기초부터 |

Python·영상편집 프로그램(프리미어 등) 지식 불필요 — 명령 한 줄씩 따라 하면 된다.

## 용어 사전

| 용어 | 뜻 (이 문서에서) |
|---|---|
| **모달리티(Modality)** | 콘텐츠의 종류. 여기서는 이미지·영상·오디오 3가지를 말한다 |
| **T2V (Text to Video)** | 글로 설명하면 영상을 만들어 주는 기능. 참조 이미지 없이 프롬프트만으로 생성 |
| **스토리보드** | 영상을 만들기 전 그리는 설계도. 씬(장면)별로 화면·대사·길이를 표로 적는다 |
| **씬(Scene)** | 영상을 이루는 한 장면 토막. 이 광고는 씬 3개(0~3초/3~7초/7~10초)로 구성 |
| **레퍼런스 고정(sref/cref)** | 같은 화풍·같은 인물을 유지하려고 기준 이미지를 지정하는 것. `sref`=스타일 기준, `cref`=캐릭터 기준 |
| **립싱크(Lip-sync)** | 말하는 입 모양과 목소리를 맞추는 것 |
| **엔드카드** | 광고 마지막에 나오는 로고·슬로건 화면 |
| **가상토큰(vt)** | 학습 플랫폼에서 AI 생성에 쓰이는 사용량 단위. 쿼터 관리용 |
| **mux (먹싱)** | 따로 만든 영상과 오디오를 하나의 파일로 합치는 작업 |

## 따라 하기

처음이라면 이 순서대로 하면 같은 결과가 나온다.

1. **기획 정하기** — 브랜드·타겟·한 줄 메시지를 먼저 적는다(§1). 이걸 안 정하면 생성 프롬프트가 흔들린다.
2. **스토리보드 표 만들기** — 씬 3개로 나누고 각 행에 화면 설명·자막·길이를 적는다(§2). 표가 곧 프롬프트 원본이 된다.
3. **이미지 생성** — 씬별 화면을 이미지 AI 로 만든다. 스타일 문구(조명·팔레트·구도)를 **모든 씬에 똑같이** 넣어야 톤이 맞는다(§3.2).
4. **영상 생성** — 움직임이 필요한 씬만 T2V 로 만든다. 도구가 8초 상한이면 씬을 나눠 만들고 나중에 붙인다.
5. **음성 만들기** — 내레이션 문장을 TTS 에 넣어 mp3 로 받는다.
6. **합치기** — ffmpeg 으로 컷을 이어 붙이고 오디오를 mux 한다(§6 파이프라인의 명령 그대로).
7. **비율 버전 만들기** — 같은 영상을 9:16 으로 크롭해 숏폼용을 하나 더 만든다.
8. **기록 남기기** — 쓴 도구·모델명·날짜·프롬프트를 README 에 적는다. 이게 없으면 재현이 안 된다.
